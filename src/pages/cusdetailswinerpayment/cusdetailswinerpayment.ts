...
/**
 * Generated class for the CusdetailswinerpaymentPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-cusdetailswinerpayment',
  templateUrl: 'cusdetailswinerpayment.html',
})
export class CusdetailswinerpaymentPage {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad CusdetailswinerpaymentPage');
    this.pid=this.navParams.get('aid');
    this.productDetails();
    this.loyaltyDetails();
  }
  ...
  productDetails(){
    ...
    this.authService.postData(serval,'myauctionpayamount').then((result) => {
      this.responseData = result
 
      if( this.responseData.Ack == 1)
      {
        loading.dismiss();
        this.packagedetails =  this.responseData.amountdetails;
...
submit(formData) {
  ...
  this.authService.auctionwinnerpayment(formData).subscribe(res => {
   if (res.Ack == 1) {
